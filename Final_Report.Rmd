...
merged_data <- final_data %>%
  ...
  mutate(total_fatalities = sum(number_of_fatalities)) %>%
  filter(row_number()==1) %>%
  select(COUNTY_NAME, total_fatalities, county_pop, 
         county_latitude, county_longitude, geometry, 
         longitude, latitude) %>%
  mutate(geometry2 = geometry) %>%
  ungroup() %>% 
  mutate(geo_id = paste(longitude, latitude, sep='_')) %>%
  group_by(geo_id) %>%
  mutate(total_lat_lon_fatals = sum(total_fatalities)) %>%
  ungroup() %>%
  as_tibble()  
...
precise_accident_data <- final_data %>%
  ...
  pal <- colorNumeric(c("white", "black"), 
                      min(merged_data$county_pop):
                      max(merged_data$county_pop)) 
  # pal <- colorNumeric(c("white", "black"), 
  ...
  leaflet(precise_accident_data) %>%
    ...
    addPolygons(
      ...
      fillColor = ~pal(merged_data$county_pop),
      highlightOptions = highlightOptions(color = "white", weight = 2,
      bringToFront = TRUE)
      ) %>%
    setView(lng=-112.033966, lat=43.491650, zoom=5) %>%
    addLegend("bottomright", pal = pal, values = ~1000*number_of_fatalities,
              title = "County Population (Thousands)\n",
              opacity = 1
    ) %>%
    ...
    addLabelOnlyMarkers(~county_longitude, ~county_latitude, label =  ~COUNTY_NAME, 
...
Location: https://en.wikipedia.org/wiki/User:Michael_J/County_table
This dataset contained the geometric centeroids of all the counties in Idaho. I gathered this data from wikipedia and cleaned this data using regular expressions.
(?=[:alpha:]\t)[^+]*
(?=[:alpha:]*\d)[^+]*
